...
module.exports = class RoomService extends Schmervice.Service {
  ...
  async findAvailableRoom(checkInAt, checkOutAt) {
    const { Bookings, Rooms } = this.server.models();
    // Cek Booking dengan rentang tanggal checkin & checkout
    const bookings = await Bookings.query()
      .whereBetween("checkInAt", [checkInAt, checkOutAt])
      .orWhereBetween("checkOutAt", [checkInAt, checkOutAt])
      .orWhere(builder => {
        builder
          .where("checkInAt", "<=", checkInAt)
          .where("checkOutAt", ">", checkInAt)
          .where("checkInAt", "<=", checkOutAt)
          .where("checkOutAt", ">", checkOutAt);
      });
